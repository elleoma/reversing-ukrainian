...
<pre spellcheck="false">cd XyZ9PlH1ZuK8
git pull
sys/user.XyZ9PlH7ZuK8
</pre>

Якщо ви раніше не дотримувалися інструкцій, необхідно побудувати radare2 з джерела, щоб це працювало, оскільки вони рідко оновлюють випуски.
...
<pre spellcheck="false">XyZ9PlH5ZuK8 -w ./0x05_asm64_float_primitive_datatype
</pre>
...
Нам потрібно змінити дві інструкції тут. Давайте розглянемо дві дуже спеціальні інструкції.

<pre spellcheck="false">movz w0, 0x999a
movk w0, 0x4121, lsl 16
</pre>

Пам'ятайте з попередньої неділі, що в кінцевому підсумку w0 буде зберігати _0x4121999a_ як _lsl_ переміщає біти в зворотньому порядку байтів.

Зараз це буде давати float _10.1_ , як ми бачили раніше в попередніх лекціях. Важливо розуміти, що в чиселах з плаваючою комою є _мантисса_ , яка в нашому випадку _10_ і _висота_ яка є _1_ , до якої вони відокремлені ._ яка поєднує їх разом.
...
Я бажаю, щоб ви взяли близько на погляд деякі приклади, які я створив для вас, щоб ви могли зрозуміти, як різні значення призводять до різних результатів. Увага! Результати будуть різними, оскільки адреси будуть різними, тому ваш ASLR буде мати різні значення.
...
Ви вже починаєте бачити шаблони тут. ВИДЕЛІТЬСЯ ВРЕМЯ І ДОСТАТИСЬ ДО ТАКИХ ЗНАЧЕНЬ, щоб краще зрозуміти, як ці значення в кінцевому підсумку потрапляють в регістр s0!

У наступному урокі ми обговоримо подвійні числа.